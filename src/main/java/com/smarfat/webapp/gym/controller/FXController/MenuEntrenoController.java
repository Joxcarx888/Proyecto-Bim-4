...
public class MenuEntrenoController implements Initializable{
    ...
    public void cargarTablaMaquinas() {
        Entreno entreno = (Entreno) tblEntrenos.getSelectionModel().getSelectedItem(); 
        if (entreno != null) {
            cargarMaquinasDelEntreno(entreno);  
        }
    }
    

    public void cargarMaquinasDelEntreno(Entreno entreno) {
        if (entreno != null && entreno.getMaquinas() != null) {
            ObservableList<Maquina> maquinas = FXCollections.observableArrayList(entreno.getMaquinas());
            tblMaquinas.setItems(maquinas);  
        } else {
